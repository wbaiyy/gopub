...
import (
	...
	"github.com/wbaiyy/gopub/src/library/common"
...
func (c *ReleaseController) enableRollBack() error {
	var ids []orm.Params
	o := orm.NewOrm()
	s, err := o.Raw("SELECT id FROM task WHERE `status`=3 and project_id = ? and  `enable_rollback`=1 ORDER BY id DESC LIMIT ?",
		c.Task.ProjectId, c.Project.KeepVersionNum).Values(&ids)
	if s > 0 && err == nil {
		idStrs := []string{}
		for _, id := range ids {
		...
		s1, err := o.Raw("SELECT link_id FROM task WHERE `enable_rollback`=1 and `id` not in ("+sqlIn+") and  project_id = ? ",
			c.Task.ProjectId).Values(&versionsRes)
		if s1 > 0 && err == nil {
			var versions []string
...
func (c *ReleaseController) failHandling(co *components.BaseComponents) {
	...
	//清理远程服务器版本库
	co.CleanUpReleasesVersion(c.Task.LinkId)
